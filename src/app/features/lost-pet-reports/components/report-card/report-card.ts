  ...
  template: `
    ...
    >
      ...
      >
        ...
        <span
          class="absolute left-3 top-3 rounded-full px-3 py-1 text-xs font-semibold shadow-[0_2px_10px_rgba(47,54,59,0.25)]"
          [class]="statusClass()"
        >
          {{ statusLabel() }}
        </span>
      ...
      <div class="flex flex-col gap-2 p-4">
        <div class="flex items-start justify-between gap-2">
          ...
          <span class="shrink-0 rounded-full bg-[var(--color-surface)] px-2 py-0.5 text-[11px] font-medium text-[var(--color-text)]">
            {{ speciesLabel() }}
          </span>
        </div>
...
export class ReportCard {
  ...
  protected readonly images = computed(() => this.report().images);
